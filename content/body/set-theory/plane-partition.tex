...
With the reflexivity, symmetry, and transivity of the same-side relation demostrated, it follows by definition that it is an equivalence relation. This result may seem trivial, but it allows for application of known properties of equivlence relations. One such property is \textit{equivalence classes}, which are the subsets for which the relation holds. For our geometry, this partitions the plane into two \textit{half-planes}.

\begin{theorem}[Partition Theorem]
    Any pair of half-planes alongwith the line forming them together cover the plane.
\end{theorem}
\begin{proof}
    First note that the relation form
\end{proof}

...
\newcommand{ \overleftrightarrow{AB}ab}[1]{\overleftrightarrow{#1}}
...
\begin{theorem}[Line Determination]
    ...
    \[ C \in  \overleftrightarrow{AB}ab{AB} \land D \in  \overleftrightarrow{AB}ab{AB} \land C \neq D \implies  \overleftrightarrow{AB}ab{CD} =  \overleftrightarrow{AB}ab{AB} \]
\end{theorem}
\begin{remark}
...
\begin{theorem}[Point Partition on a Line]
    For any distinct points $A$ and $B$, the point $A$ partitions the line $ \overleftrightarrow{AB}ab{AB}$ into the point $A$ itself and two mutually disjoint open rays.
    \[  \overleftrightarrow{AB}ab{AB} \setminus \{A\} = \{X \in \Pset \mid \B(X, A, B)\} \cup \{X \in \Pset \mid \B(A, X, B) \lor \B(A, B, X)\} \]
\end{theorem}
...
\begin{theorem}[Parallelism to a Sub-ray]
    Let $l \in \Lset$ be a line and $\ray{AB}$ be a ray. We say $\ray{AB} \parallel l$ if and only if $ \overleftrightarrow{AB}ab{AB} \parallel l$.
\end{theorem}
...
\begin{itemize}
    ...
    \item \textbf{Ray Parallelism:} In formal verifications, ray parallelism (co-directionality) is often more useful than line parallelism. Two rays $\ray{AB}$ and $\ray{CD}$ are parallel if they do not intersect, and they lie on the same ``side'' of the line $ \overleftrightarrow{AB}ab{AC}$. This introduces the concept of ``sides of a line'' (half-planes), which intrinsically requires Pasch's Axiom to prove.
